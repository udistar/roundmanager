...
- **Windy에서 보기 버튼**: `https://www.windy.com/{lat}/{lon}?{lat},{lon},11` (Windy 공식 URL 형식, https://community.windy.com/topic/77/windy-com-url-parameters) 을 새 탭으로 연다. www.windy.com 은 Android App Links / iOS Universal Links 를 등록해 두어 앱이 설치돼 있으면 Windy 앱으로 열린다(커스텀 windy:// 스킴은 공식 제공 안 함). Windy Premium(개인 구독)에는 API 사용권이 없고 Point/Map Forecast API 는 별도 Professional 라이선스(각 990 €/년)라 코드에서 쓰지 않는다.
